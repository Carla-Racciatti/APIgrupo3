...
const getLibros = async (req = request, res = response) => {
  try {
    ...
    const libros = docs.map(({ key, title, author_name : authorName, first_publish_date: firstPublishDate, first_publish_year: firstPublishYear }) => ({
      key,
      title,
      authorName: author_name || ['Autor desconocido'], // Proporciono esta opción ya que muchos libros no tienen su autor. Open Library  no es constante en los registros
